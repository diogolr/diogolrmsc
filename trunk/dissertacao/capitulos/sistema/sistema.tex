...
\dot{m} = \rhoagua\ A\dot{L} = 
          \rhoagua\ Q_{\text{\tiny IN}} - 
          \rhoagua\ \underbrace{a\upsilon_{\text{\tiny OUT}}}_
                     {Q_{\text{\tiny OUT}}}
\end{equation}
...
SISO e passa a ser MIMO. Assim sendo, esse sistema será o utilizado como modelo
de estudo de caso no trabalho. A única modificação existente nas equações que
regem a dinâmica do modelo é a introdução de uma nova variável na equação do
...
% ------------------------------------------------------------------------------
\subsection{Limitações do processo}\label{sec:limitacoes}
...
iterativos para a aproximação numérica de soluções de equações diferenciais
...
alterado de 1,0 para qualquer outro valor a escolha do usuário, no momento em que é
...
A primeira rede seria a responsável pela identificação da dinâmica de $T_1$,
...
Acoplar tal sistema a um ambiente de monitoramento e supervisão, ou ainda
...
programação, a biblioteca Qt para o desenvolvimento das interfaces gráficas e a
biblioteca {\it Flood} para a implementação das redes neurais.

O primeiro sistema, cuja captura de tela ({\it screenshot}) pode ser vista na
Fig. \ref{fig:captura_simulador}, tinha por objetivo simular o funcionamento do
sistema de tanques. Para isso, fez-se uso do método de RK4 e da utilização de
arquivos de texto simples para realizar a configuração de todos os parâmetros
referentes a cada uma dessas falhas. Os parâmetros de configuração são
especificados em cada uma das colunas do arquivo de entrada, conforme Tab.
...
\small
\centering
\caption{Especificação das colunas do arquivo de configuração da simulação.}
...
Esse sistema também implementa as rotinas de dois controladores (um para cada
...
\subfigure[]
{
    \includegraphics[width=0.25\textwidth]{imgs/sistema/eps/controladores}
    \label{fig:captura_controladores}
}
\quad
\subfigure[]
{
    \includegraphics[width=0.25\textwidth]{imgs/sistema/eps/ip_porta}
    \label{fig:captura_ip_porta}
}
\caption{Captura de tela dos campos de ajuste dos controladores, IP e porta.}
\end{figure}
...
sockets} TCP, se comunicando com um servidor implementado conforme descrito em
...
de saída do primeiro sistema (simulador) e exibir as detecções das falhas de
maneira simples e intuitiva. Para isso, as matrizes de saída do simulador
deverão ser pré-processadas para que se faça uma adequação das variáveis àquelas
utilizadas por cada um dos módulos de detecção e/ou diagnóstico. 

\Glossary{Simddef}{Sistema modular para detecção e diagnóstico de falhas}
...
carregados a partir desses arquivos que seguem uma formatação pré-estabelecida,
...
\Glossary{XML}{eXtensible Markup Language}
