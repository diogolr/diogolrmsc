...
\begin{itemize}
    ...
    \begin{itemize}
        \item Proposta 1: N\textordmasculine\ palavras \implica 
                          $\uparrow$N\textordmasculine\ bits e
                          $\uparrow$N\textordmasculine\ combinações
        \item Não há decodificação da palavra
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item Estabelecimento de valores para cada falha
    \begin{itemize}
        \item Ruído: $\pm 2\%$
        \item $K_m' = 0.8 K_m$
        \item ...
    \end{itemize}
    \item Simulação do sistema sem falhas
    \item Simulação com falhas (individual)
    \item Aquisição dos dados
    \item Processamento (treinamento das redes)
    \item Validação
\end{itemize}
\end{frame}
