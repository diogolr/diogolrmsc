...
utilizados modelos NNARX em que o número de regressores e o número de neurônios
na camada oculta foram determinados a partir de testes, conforme mostrado no
Cap. \ref{cap:resultados}. Os detalhes relativos a cada uma dessas estruturas
são esclarecidos nas próximas seções.
...
\begin{figure}[htb]
\centering
    \includegraphics[width=\textwidth]{imgs/sistema/eps/detec_prop_1}
...
\begin{figure}[htb]
\centering
    \includegraphics[width=\textwidth]{imgs/sistema/eps/detec_prop_2}
...
estrutura mais complexa, o treinamento poderia ter um grande custo com relação
ao tempo e ao processamento. Na segunda proposta apenas uma das redes seria
retreinada, o que poderia reduzir consideravelmente esse tempo de treinamento e
processamento. Além disso, se o treinamento fosse realizado em tempo real, não
haveria necessidade de se parar o sistema de detecção como um todo, mas somente
de uma parte deste. 

Uma outra vantagem estaria relacionada com o pós processamento da informação. Em
um primeiro momento ao se utilizar a segunda proposta não seria necessário que
fosse realizada uma decodificação da saída como acontece na primeira. Conforme
dito anteriormente, a palavra gerada na saída das redes especialistas indica se
aquela falha está ou não acontecendo em $T_1$, $T_2$ ou em $T_1$ e $T_2$
simultaneamente. 
...
vez, está diretamente relacionado com o número de falhas a serem identificadas,
...
(setenta e oito) combinações distintas. Se fossem consideradas todas as
possibilidades esse número cresceria consideravelmente.
...
Um último ponto que pode ser destacado é que ao se utilizar a segunda proposta,
torna-se mais fácil realizar o acoplamento do sistema de DDF com um sistema de
monitoramento e geração de alarme. Se cada saída da ...
